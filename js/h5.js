...
const h5Notes = [
  ...
  },
  {
    title: "图像标签",
    content: [  
    "gif，jpeg，png JPG (JPEG) ,区别，JPG <br>压缩方式：: 有损压缩，会牺牲部分画质以减小文件大小。 <br>适用场景：: 数码照片、色彩丰富的图像，非常适合保存大量图片，因为文件较小。<br> 缺点：: 画质损失，不适合对颜色和细节要求极高的场景，不支持透明背景。<br> PNG 压缩方式：: 无损压缩，保留所有图像细节，画质好。 <br>适用场景：: 网页设计中的图标、Logo、需要透明背景的图片，以及需要二次编辑的源文件。 <br>缺点：: 文件大小通常比JPG 大。 优点：: 支持透明背景（包括Alpha 通道，可调节透明度），画质比JPG 更好。<br>GIF 压缩方式：: 无损压缩，但颜色数量限制在256 色。 <br>适用场景：: 简单的动画效果、小图标。 <br>缺点：: 颜色数量少，不适合展示色彩丰富的图像，动画效果简单。 优点：: 支持简单的动画，支持透明背景。 PNG-8 是GIF 的改进版本，在透明度和色彩方面通常优于GIF，且不支持动画<br>" ,
    "&lt;hr width=80%&gt; <br>给hr添加宽度属性展示如下 <hr width=80%>  <br> ",
    "&lt;hr  width=10% align='center' &gt; <br>给hr添加对齐属性,展示如下 <hr width=10% align='center'>  <br>"
    ],
    timestamp: "2025-10-17 08:30",  
