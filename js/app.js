...
for (const category in subjectCategories) {
    for (const subject in subjectCategories[category]) {
        subjectChapters[subject] = subjectCategories[category][subject];
    }
}

// 辅助函数：获取所有分类
function getAllCategories() {
    return Object.keys(subjectCategories);
}

// 辅助函数：根据分类获取学科列表
function getSubjectsByCategory(category) {
    return subjectCategories[category] ? Object.keys(subjectCategories[category]) : [];
}

// 辅助函数：获取学科所属分类
function getCategoryOfSubject(subject) {
    for (const category in subjectCategories) {
        if (subject in subjectCategories[category]) {
            return category;
        }
    }
    return null;
...
const NOTE_CACHE_VERSION = "v20260118"; // 格式：v+日期/迭代号，如 v20251031、v3
...
function generateSubjectButtons() {
    ...
    // 如果URL参数指定了学科且该学科存在
    if (urlParams.subject && subjectChapters[urlParams.subject]) {
        activeSubject = urlParams.subject;
        activeCategory = getCategoryOfSubject(activeSubject);
    }
    ...
    if (!activeCategory && categories.length > 0) {
        ...
        // 找到该分类的第一个学科作为激活学科
        const firstSubjects = Object.keys(subjectCategories[activeCategory]);
        if (firstSubjects.length > 0) {
            activeSubject = firstSubjects[0];
        }
    }
    ...
    // 存储每个分类的第一个学科
    const categoryFirstSubjects = {};
    
    // 遍历每个分类
    categories.forEach(category => {
        // 创建分类选项卡项
        const tabItem = document.createElement('button');
        tabItem.className = 'category-tab-item';
        tabItem.dataset.category = category;
        ...
        categoryButtons.dataset.category = category;
        ...
        // 存储按钮容器引用
        categoryButtonsMap[category] = categoryButtons;
        
        // 获取该分类下的所有学科
        const subjects = Object.keys(subjectCategories[category]);
        
        // 记录该分类的第一个学科
        if (subjects.length > 0) {
            categoryFirstSubjects[category] = subjects[0];
        }
        ...
        // 将分类按钮容器添加到内容区域
        categoryContent.appendChild(categoryButtons);
        
        // 分类选项卡点击事件
        tabItem.addEventListener('click', () => {
            // 检查当前激活的分类
            const currentActiveTab = document.querySelector('.category-tab-item.active');
            const isSameCategory = currentActiveTab && currentActiveTab.dataset.category === category;
            ...
            // 显示当前分类对应的按钮容器
            if (categoryButtonsMap[category]) {
                categoryButtonsMap[category].classList.add('active');
            }
            
            // 如果切换到不同的分类，激活该分类下的第一个学科
            if (!isSameCategory) {
                const firstSubject = categoryFirstSubjects[category];
                if (firstSubject) {
                    // 找到该学科按钮并模拟点击
                    const subjectBtn = categoryButtonsMap[category].querySelector(`.subject-btn[data-subject="${firstSubject}"]`);
                    if (subjectBtn) {
                        subjectBtn.click();
                    }
                }
    ...
    // 将选项卡和内容添加到分类组
    categoryGroup.appendChild(categoryTabs);
    categoryGroup.appendChild(categoryContent);
    
    // 将分类组添加到按钮容器
    buttonContainer.appendChild(categoryGroup);
}
...
function fetchChapterTitle(subject, chapter, element) {
    // 导入该学科的章节标题文件
    const category = getCategoryOfSubject(subject);
    import(`../notes/${category}/${subject}/chapters.js?v=${NOTE_CACHE_VERSION}`)
        .then(module => {
            const chapterTitles = module.default; // 获取章节标题数组
            // 获取对应章节的标题（章节号从1开始，数组索引从0开始）
            const title = chapterTitles[chapter - 1] || `第${chapter}章`;
            // 同时加载该章节的笔记数量
            import(`../notes/${category}/${subject}/chapter${chapter}.js?v=${NOTE_CACHE_VERSION}`)
                .then(noteModule => {
