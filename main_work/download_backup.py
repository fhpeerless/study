...
{sections}
    ...
    <div class="password-modal" id="passwordModal">
        <div class="password-modal-content">
            <h3 class="password-modal-title"><i class="fa fa-lock"></i> 请输入下载密码</h3>
        ...
        document.querySelectorAll('.download-btn[data-url]').forEach(btn => {{
            btn.addEventListener('click', function(e) {{
                ...
                if (passwordPageId && passwordPageId !== 'false') {{
                    ...
                    const pageName = getPasswordPageName(passwordPageId);
                    document.querySelector('.password-modal-title').innerHTML = `<i class="fa fa-lock"></i> 请输入${{pageName}}`;
